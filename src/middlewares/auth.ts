...
const ADMIN_EMAILS: string[] = process.env.ADMIN_EMAILS ? JSON.parse(process.env.ADMIN_EMAILS) : [];
...
export const auth = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    return ADMIN_EMAILS.includes(decoded.email) ? next() : next(new HttpException(401, `Unauthorized: not admin user`));
  } catch (e) {
